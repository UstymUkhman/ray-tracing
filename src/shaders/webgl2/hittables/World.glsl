...
{
  for (float i = -11.0; i < 11.0; i++)
  {
    for (float j = -11.0; j < 11.0; j++)
    {
      ...
      { */
        ...
        if (rand < 0.8)
        {
          vec3 color = randomVec(seed * i) * randomVec(seed * j);
          addSphere(0.2, center, Material(LAMBERTIAN, color, 0.0));
        }
        else if (rand < 0.95)
        {
          float fuzz = random(seed * rand, 0.0, 0.5);
          vec3 color = randomVec(seed * fuzz, 0.5, 1.0);
